...
1. **From the chat** — just ask. *"Write me a Python script that pings a list of servers and prints any that are unreachable."*
...
> *"A shell script for Ubuntu that rotates `/var/log/myapp/*.log` files older than 7 days, gzips them, and writes a one-line summary to stdout."*

> *"A PowerShell script that checks the status of the `Spooler` service and restarts it if it's stopped. Print before/after state."*
...
When Autobot creates a script, you'll see an inline **tool badge** in the chat — e.g. *"Creating script: `ping_servers.py`"* — and the script is saved into your Script library as it happens.
...
> *"Now add a `--timeout` argument that defaults to 2 seconds."*

> *"Make it write the unreachable hosts to a file instead of stdout."*
...
> *"Run `disk-check.py` on `prod-db-1` using the `db-admin` credential."*
